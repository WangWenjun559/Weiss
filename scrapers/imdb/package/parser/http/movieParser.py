...
class DOMHTMLMovieIdParser(DOMParserBase):
    """Parser for the "news" page of a given movie or person.
    The page should be provided as a string, as taken from
    the akas.imdb.com server.  The final result will be a
    dictionary, with a key for every relevant section.
    ...
    extractors = [
        Extractor(label='id',
            path="//td[@class='title']",
            attrs=Attribute(key='id',
                multi=True,
                path={
                    'id': "./preceding-sibling::td[1]/a/@href"
                   },
                postprocess=lambda x: {
                    'id': (x.get('id').strip()[9:-1] or u'')
                })),
         Extractor(label='amount',
            path="//div[@id='left']",
            attrs=Attribute(key='amount',
                multi=True,
                path={
                    'amount': "./text()"
                   },
                postprocess=lambda x: {
                    'amount': (_id_amount_paser(x.get('amount')))
                }))
        ]
...
_id_amount_p = re.compile('([0-9|,]+)\ntitles')
def _id_amount_paser(s):
    s = _id_amount_p.findall(s)[0]
    res = filter(lambda x: x != ',', s)
    return res


class DOMHTMLUserReviewParser(DOMParserBase):
    """Parser for the "news" page of a given movie or person.
    The page should be provided as a string, as taken from
    the akas.imdb.com server.  The final result will be a
    dictionary, with a key for every relevant section.
    ...
    extractors = [
        Extractor(label='reviews',
            path="//h2",
            attrs=Attribute(key='reviews',
                multi=True,
                path={
                    'title': "./text()",
                    # FIXME: sometimes (see The Matrix (1999)) <p> is found
        ...
        Extractor(label='amount',
            ...
            attrs=Attribute(key='amount',
                ...
                postprocess=lambda x: {
                    'amount': _parse_reviews_amount(x.get('amount').strip())
                }))
        ]
...
def _parse_rating(s):
    if (s is None):
        return None
    outof = s.find(' out of ')
    people = s.find(' people')
    norm = float(s[:outof])
    denorm = float(s[outof + 8:people])
    return str(norm/denorm)
...
_OBJECTS = {
    ...
    'reviews_parser':  ((DOMHTMLUserReviewParser,), None),  ## Ming: for user reviews
    'id_parser':  ((DOMHTMLMovieIdParser,), None),  ## Ming: for movie ids
